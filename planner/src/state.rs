...
impl ResolutionContext for State {
        ...
        predicate
            ...
            .all(|(av, v)| match av {
                ...
                Value::ActionParam(ap) => objects
                    .get(*v)
                    .is_some_and(|o| ap.r#type.container().inherits_or_eq(&o.r#type, &ap.r#type)),
            })
    }
    ...
    fn resolve_predicate(
        ...
        self.predicates
            .iter()
            .filter(|rp| self.is_resolution_of(predicate, rp, objects))
            .cloned()
            .collect()
    }
    ...
    fn resolve_negated_predicate(
        ...
        let objects_for_param = resolve_pred_as_parameter_map(&pos_predicate, || {
            self.resolve_predicate(&pos_predicate, objects)
        });
    ...
    fn resolve_action(&self, action: Action, objects: &ObjectCollection) -> ActionResolution {
        let handle_primitives = |p: &Primitives| match p {
            Primitives::Not(np) => {
                resolve_pred_as_parameter_map(&np.o, || self.resolve_negated_predicate(np, objects))
            }
            Primitives::Pred(p) => {
                resolve_pred_as_parameter_map(p, || self.resolve_predicate(p, objects))
            }
        };
...
mod tests {
    ...
    fn test_preciate_resolution() {
        ...
        assert_eq!(
            state.resolve_predicate(&ps, &objects),
        ...
        assert_eq!(
            state.resolve_predicate(&pp, &objects),
        ...
        assert_eq!(
            state.resolve_negated_predicate(&Not::new(&pp2), &objects),
            BTreeSet::from([
                p2.as_resolved(&[x]),
                p2.as_resolved(&[y]),
                p2.as_resolved(&[b])
        ...
        assert_eq!(state.resolve_predicate(&p3, &objects), BTreeSet::default());
        ...
        assert_eq!(
            state.resolve_negated_predicate(&Not::new(&p4), &objects),
            BTreeSet::from([
                p4.as_resolved(&[x]),
                p4.as_resolved(&[y]),
                p4.as_resolved(&[b]),
    ...
    pub fn test_action_resolution_with_explicit_predicates() {
        ...
        let res = state.resolve_action(action, &objects);
        assert!(!res.is_empty());

        let correct_resolution = BTreeSet::from([BTreeMap::from([
            (action_params[0].clone(), BTreeSet::from([x])),
            (action_params[1].clone(), BTreeSet::from([y])),
        ])]);
    ...
    pub fn test_impossible_resolution_with_partially_defined_predicate() {
        ...
        let action = ActionBuilder::new("action")
            ...
            .precondition(|params| {
                Formula::new(FM::and(&[
                    FM::pred(
                        p1.as_specific(&[Value::ActionParam(params[0].clone()), Value::Object(x2)]),
                    ),
            ...
            .effect(|params| {
                And::new(&[
                    Pr::not(
                        p1.as_specific(&[Value::Object(x2), Value::ActionParam(params[0].clone())]),
                    ),
                    Pr::pred(
                        p2.as_specific(&[Value::ActionParam(params[1].clone()), Value::Object(y1)]),
                    ),
                ])
            })
            .build();
    ...
    pub fn test_action_resolution_with_subtypes() {
        ...
        let correct_resolution = BTreeSet::from([
            ...
            BTreeMap::from([(action_params[0].clone(), BTreeSet::from([x1, x2, xx1, xx2]))]),
